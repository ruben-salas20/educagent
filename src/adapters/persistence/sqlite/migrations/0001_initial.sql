...
-- 0001_initial.sql
-- ============================================
-- Schema inicial: 16 tablas + 1 view + 2 triggers append-only.
-- Fuente de verdad: docs/architecture/02-schema-sqlite.md §"DDL completo".
--
-- NOTA: el bloque `CREATE TABLE _migrations` del blueprint usa IF NOT EXISTS
-- porque el runner (migrations/runner.ts) crea esa tabla on-demand antes de
-- aplicar cualquier migration. Es el único delta vs el documento — ningún
-- campo, CHECK, índice o trigger fue modificado.
...
-- ---- Migrations registry ----
-- IF NOT EXISTS: el runner crea esta tabla on-demand antes de aplicar la migration.
